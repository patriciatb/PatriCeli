    ...
    public String crearZona(@RequestBody DatosTwitter dz) { //dz es un array list auxiliar
        ...
        try {
            for (int i = 0; i < db.getDatos().size(); i++) { // recorro el json entero
                if (db.getDatos().get(i).getCodigo_geometria().equals(dz.getCodigo_geometria())) { // si el código de ID ya existe en la base de datos
        ...
        String id = db.getDatos().get(db.getDatos().size() - 1).getCodigo_geometria(); // obtengo el id del último elemento de la lista
        int codigoInt = Integer.parseInt(id); // lo convierto a entero
        codigoInt++; // le sumo 1
        id = String.valueOf(codigoInt); // lo convierto a String
        ...
        dz.setCodigo_geometria(id); // le asigno el nuevo id al objeto dz
        ...
        return "Zona básica con código " + dz.getCodigo_geometria() + " añadido a la base de datos";
    }
//-----------------------------------------------------------------
    ...
    public String eliminar(@RequestBody String id) {
        ...
        for (int i = 0; i < db.getDatos().size(); i++) {
            ...
            System.out.println("El id de la base de datos es: " + db.getDatos().get(i).getCodigo_geometria());
            // if que compare el id que recibe con el id de la base de datos
            if (db.getDatos().get(i).getCodigo_geometria().equals(id)) { //comparo los datos
                // Imprime el dato que se va a eliminar
                System.out.println("El dato que se va a eliminar es: " + db.getDatos().get(i).getfecha_informe());
